 ...
 */
case class SessionizedBroker(id: Int, endPoints: Seq[EndPoint], rack: Option[String], sessionId: Long) {
  ...
  def this(id: Int, host: String, port: Int, listenerName: ListenerName, protocol: SecurityProtocol, sessionId: Long) = {
    this(id, Seq(EndPoint(host, port, listenerName, protocol)), None, sessionId)
  }

  def this(bep: BrokerEndPoint, listenerName: ListenerName, protocol: SecurityProtocol, sessionId: Long) = {
    this(bep.id, bep.host, bep.port, listenerName, protocol, sessionId)
  }
