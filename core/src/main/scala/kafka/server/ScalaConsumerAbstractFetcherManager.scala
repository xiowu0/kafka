...
/**
  *
  * AbstractFetcherManager from 2.0 release. Renaming it to ScalaConsumerAbstractFetcherManager and
  * add it back to support scala consumer.
  */
abstract class ScalaConsumerAbstractFetcherManager(protected val name: String, clientId: String, numFetchers: Int = 1)
  extends Logging with KafkaMetricsGroup {
  // map of (source broker_id, fetcher_id per source broker) => fetcher.
  // package private for test
  private[server] val fetcherThreadMap = new mutable.HashMap[BrokerIdAndFetcherId, ScalaConsumerAbstractFetcherThread]
  ...
  def createFetcherThread(fetcherId: Int, sourceBroker: BrokerEndPoint): ScalaConsumerAbstractFetcherThread
