...
object ConsumerConfig extends Config {
  ...
  def validateAutoOffsetReset(autoOffsetReset: String) {
    autoOffsetReset match {
      ...
      case _ => throw new InvalidConfigurationException("Wrong value " + autoOffsetReset + " of auto.offset.reset in ConsumerConfig; " +
  ...
  def validateOffsetsStorage(storage: String) {
    storage match {
      case "zookeeper" =>
      case "kafka" =>
      case _ => throw new InvalidConfigurationException("Wrong value " + storage + " of offsets.storage in consumer config; " +
  ...
  def validatePartitionAssignmentStrategy(strategy: String) {
    strategy match {
      case "range" =>
      case "roundrobin" =>
      case _ => throw new InvalidConfigurationException("Wrong value " + strategy + " of partition.assignment.strategy in consumer config; " +
