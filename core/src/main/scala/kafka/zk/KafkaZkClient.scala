...
class KafkaZkClient private[zk] (zooKeeperClient: ZooKeeperClient, isSecure: Boolean, time: Time) extends AutoCloseable with
  ...
  def registerControllerAndIncrementControllerEpoch(controllerId: Int): (Int, Int) = {
    ...
    def tryCreateControllerZNodeAndIncrementEpoch(): (Int, Int) = {
      val response = retryRequestUntilConnected(
        MultiRequest(Seq(
          CreateOp(ControllerZNode.path, ControllerZNode.encode(controllerId, timestamp), acls(ControllerZNode.path), CreateMode.EPHEMERAL),
   ...
   */
  def setTopicAssignment(topic: String, assignment: Map[TopicPartition, Seq[Int]], expectedControllerEpochZkVersion: Int = ZkVersion.NoVersion) = {
  ...
  def deletePartitionReassignment(): Unit = {
    deletePath(ReassignPartitionsZNode.path, ZkVersion.NoVersion)
  }
   ...
   */
  def deletePath(path: String, expectedControllerEpochZkVersion: Int = ZkVersion.NoVersion, recursiveDelete: Boolean = true): Unit = {
    if (recursiveDelete)
   ...
   */
  def deleteRecursive(path: String, expectedControllerEpochZkVersion: Int = ZkVersion.NoVersion): Boolean = {
    val getChildrenResponse = retryRequestUntilConnected(GetChildrenRequest(path))
    getChildrenResponse.resultCode match {
      case Code.OK =>
        getChildrenResponse.children.foreach(child => deleteRecursive(s"$path/$child", expectedControllerEpochZkVersion))
        val deleteResponse = retryRequestUntilConnected(DeleteRequest(path, ZkVersion.NoVersion), expectedControllerEpochZkVersion)
        if (deleteResponse.resultCode != Code.OK && deleteResponse.resultCode != Code.NONODE)
          throw deleteResponse.resultException.get
        true
      case Code.NONODE => false
  ...
  private def retryRequestsUntilConnected[Req <: AsyncRequest](requests: Seq[Req], expectedControllerZkVersion: Int): Seq[Req#Response] = {
    expectedControllerZkVersion match {
      case ZkVersion.NoVersion => retryRequestsUntilConnected(requests)
      ...
      case invalidVersion =>
        throw new IllegalArgumentException(s"Expected controller epoch zkVersion $invalidVersion should be non-negative or equal to ${ZkVersion.NoVersion}")
    }
  }
