...
class CheckpointPersistentCacheTest extends JUnitSuite with Logging {
  ...
  def shouldPersistAndOverwriteAndReloadFile(): Unit = {
    ...
    val tp2 = new TopicPartition("topic2", 234)

    checkpointStore.update(Map(tp1 -> 55L, tp2 -> 19242342458L))
    checkpointStore.persist()

    assertEquals(55L, checkpointStore.getCheckpoint(tp1).get)
    ...
    assertEquals(55L, checkpointStore2.getCheckpoint(tp1).get)

    // overwrite
    checkpointStore.update(Map(tp1 -> 55L))
    checkpointStore.persist()

    val checkpointStore3 = new CheckpointPersistentCacheFile[TopicPartition, Long](
      checkpointFile,
      OffsetCheckpointFile.Formatter)
    assertEquals(55L, checkpointStore3.getCheckpoint(tp1).get)
    assertTrue(checkpointStore3.getCheckpoint(tp2).isEmpty)
  }
}
