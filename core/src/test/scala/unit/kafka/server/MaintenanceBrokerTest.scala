...
class MaintenanceBrokerTest extends ZooKeeperTestHarness {
  ...
  def testTopicCreatedByAdminClientShouldHonorMaintenanceBrokers(): Unit = {
    ...
    val future2 = client.createTopics(Seq("topic2").map(new NewTopic(_, 3, 2)).asJava,
      new CreateTopicsOptions()).all()
    future2.get()
    ...
    val future3 = client.createTopics(Seq("topic3").map(new NewTopic(_, 3, 1)).asJava,
      new CreateTopicsOptions()).all()
    future3.get()
    ...
    val future4 = client.createTopics(Seq("topic4").map(new NewTopic(_, 3, 3)).asJava,
      new CreateTopicsOptions()).all()
    future4.get()
    ...
    // create topic using admin client
    val future5 = client.createTopics(Seq("topic5").map(new NewTopic(_, 3, 1)).asJava,
      new CreateTopicsOptions()).all()
    future5.get()
